...
function buildLedgerRows(workRows: WorkRow[], payRows: PaymentRow[], t: (key: string) => string): LedgerRow[] {
  ...
  return all.map(item => {
    if (item.src === "work") {
      ...
      balance += credited;
      const statusLabel = w.status === "present" ? t("statements.present") : w.status === "half_day" ? t("statements.halfDay") : t("statements.absent");
      const workerInfo = w.num_labourers ? `${w.num_labourers} ${t("statements.workers").toLowerCase()}` : w.hours_worked ? `${w.hours_worked}h` : "";
      return {
        date: w.date,
        rowType: "work" as const,
        description: w.task ? `${statusLabel} — ${w.task}` : `${t("statements.dailyWorkStatus")} (${statusLabel})`,
        status: w.status,
...
function generateStatementHTML(
  ...
  const typeLabel =
    statementType === "work" ? t("statements.workStatementTitle") :
    statementType === "payment" ? t("statements.paymentStatementTitle") :
    t("statements.combinedStatementTitle");

  const periodLabel = `${dateFrom ? fmtDate(dateFrom) : "—"} ${t("common.to").toLowerCase()} ${dateTo ? fmtDate(dateTo) : "—"}`;
  ...
  // ── entity info rows ──
  const infoRows = [
    [t("statements.name"), entity.name],
    [t("statements.entityType"), isTeam ? t("statements.team") : t("statements.individualLabour")],
    entity.hometown ? [t("statements.hometownOrigin"), entity.hometown] : null,
    entity.phone ? [t("statements.phone"), entity.phone] : null,
    entity.aadhaar ? [t("statements.aadhaar"), entity.aadhaar] : null,
    [t("statements.dailyWage"), fmtCur(entity.daily_wage)],
    entity.car_rent ? [t("statements.carRent"), fmtCur(entity.car_rent)] : null,
    entity.manager_fee ? [t("statements.managerFee"), fmtCur(entity.manager_fee)] : null,
    entity.member_count !== undefined ? [t("statements.teamMembers"), String(entity.member_count)] : null,
  ].filter(Boolean) as [string, string][];

  // ── summary rows ──
  const summaryItems = [
    `<span>${t("statements.daysPresent")}: <b>${summary.total_days_present}</b></span>`,
    `<span>${t("statements.daysAbsent")}: <b>${summary.total_days_absent}</b></span>`,
    `<span>${t("statements.halfDays")}: <b>${summary.total_days_half}</b></span>`,
    `<span>${t("statements.totalEarned")}: <b>${fmtCur(summary.total_wage_earned)}</b></span>`,
    `<span>${t("statements.totalBorrowed")}: <b>${fmtCur(summary.total_paid_borrowed)}</b></span>`,
    `<span style="color:${summary.balance >= 0 ? "#166534" : "#991b1b"}">${t("statements.balance")}: <b>${fmtCur(Math.abs(summary.balance))} ${summary.balance < 0 ? `(${t("statements.overpaid")})` : summary.balance === 0 ? `(${t("statements.settled")})` : `(${t("statements.pending")})`}</b></span>`,
  ];
  ...
  if (statementType === "work") {
    ...
    const rows = workRows.map((r, i) => {
      const statusLabel = r.status === "present" ? t("statements.present") : r.status === "half_day" ? t("statements.halfDay") : t("statements.absent");
      const statusColor = r.status === "present" ? "#166534" : r.status === "half_day" ? "#92400e" : "#991b1b";
      const workerInfo = isTeam && r.num_labourers ? `${r.num_labourers} ${t("statements.workers").toLowerCase()}` : r.hours_worked ? `${r.hours_worked}h` : "—";
      ...
      <table style="width:100%;border-collapse:collapse;font-family:Inter,sans-serif;">
        <thead>
          <tr style="background:${headerBg};color:#ffffff;">
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:left;white-space:nowrap;">${t("statements.date")}</th>
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:left;">${t("statements.status")}</th>
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:left;">${t("statements.taskDescription")}</th>
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:center;">${isTeam ? t("statements.workers") : t("statements.hours")}</th>
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:right;">${t("statements.wageEarned")}</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
        <tfoot>
          <tr style="background:#f0fdf4;border-top:2px solid #1b4332;">
            <td colspan="4" style="padding:11px 14px;font-size:13px;font-weight:700;color:#1b4332;">${t("statements.totalWageEarned").toUpperCase()}</td>
    ...
    if (statementType === "payment") {
      ...
      // Combined
      ledgerRows = buildLedgerRows(workRows, payRows, t);
    }
    ...
    const rows = ledgerRows.map((r, i) => {
      ...
      const typeLabel = isWork
        ? `<span style="font-size:11px;font-weight:600;padding:2px 7px;border-radius:9999px;background:#dcfce7;color:#166534;">${t("statements.wage")}</span>`
        : `<span style="font-size:11px;font-weight:600;padding:2px 7px;border-radius:9999px;background:#fee2e2;color:#991b1b;">${t("statements.advance")}</span>`;
    ...
    const typeColHeader = statementType === "combined"
      ? `<th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:left;">${t("statements.typeColumn")}</th>`
      : "";
      ...
      <table style="width:100%;border-collapse:collapse;font-family:Inter,sans-serif;">
        <thead>
          <tr style="background:${headerBg};color:#ffffff;">
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:left;white-space:nowrap;">${t("statements.date")}</th>
            ${typeColHeader}
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:left;">${t("statements.description")}</th>
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:right;white-space:nowrap;">${t("statements.creditedPlus")}</th>
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:right;white-space:nowrap;">${t("statements.borrowedMinus")}</th>
            <th style="padding:11px 14px;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;text-align:right;">${t("statements.balance")}</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
        <tfoot>
          <tr style="background:#f0fdf4;border-top:2px solid #1b4332;">
            <td colspan="${statementType === "combined" ? 3 : 2}" style="padding:11px 14px;font-size:13px;font-weight:700;color:#1b4332;">${t("statements.totals").toUpperCase()}</td>
  ...
  const emptyMsg = workRows.length === 0 && payRows.length === 0
    ? `<p style="text-align:center;color:#9ca3af;padding:32px;font-size:14px;">${t("statements.noRecordsFound")}</p>`
    : "";
...
<div style="margin-bottom:24px;">
  <p style="font-size:11px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:#6b7280;margin-bottom:6px;">${t("statements.farmBrand")}</p>
...
<td style="vertical-align:top;text-align:right;width:40%;">
  <div style="margin-bottom:5px;">
    <span style="font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:#6b7280;">${t("statements.period")}</span><br>
    <span style="font-size:13px;font-weight:600;color:#111827;">${periodLabel}</span>
  </div>
  <div style="margin-top:10px;">
    <span style="font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:#6b7280;">${t("statements.generated")}</span><br>
...
<div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:14px 18px;margin-bottom:32px;">
  <p style="font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:#6b7280;margin-bottom:8px;">${t("statements.statementSummary")}</p>
...
<div style="border-top:1px solid #e5e7eb;padding-top:14px;display:flex;justify-content:space-between;align-items:center;">
  <p style="font-size:11px;color:#9ca3af;">${t("statements.footerComputerGenerated")}</p>
  <p style="font-size:11px;color:#9ca3af;">${t("statements.statementForFooter")} <b style="color:#374151;">${entity.name}</b></p>
</div>
...
export default function StatementsPage() {
  ...
  // Track which type was actually fetched (used for rendering view correctly)
  const [fetchedType, setFetchedType] = useState<StatementType>("combined");
  ...
  const fetchStatement = useCallback(async () => {
    ...
    setError(null);
    setCurrentPage(1);
    // Capture the type at fetch time to avoid stale closure issues
    const typeToFetch = statementType;
    try {
      const params: Record<string, string> = {};
      if (dateFrom) params.date_from = dateFrom;
      if (dateTo) params.date_to = dateTo;
      const data = await apiGet<AnyStatement>(
        `/api/v1/statements/${typeToFetch}/${selectedEntity.entity_type}/${selectedEntity.id}`,
        params
      );
      setStatement(data);
      setFetchedType(typeToFetch);
      setStep("view");
    } catch {
      setError(t("statements.failedToLoad"));
    } finally {
      setLoading(false);
    }
  }, [selectedEntity, statementType, dateFrom, dateTo, t]);

  // Derived data — use fetchedType to match what the API actually returned
  const workRows: WorkRow[] = (() => {
    if (!statement) return [];
    if (fetchedType === "combined" && "work_rows" in statement) return (statement as CombinedStatement).work_rows;
    if (fetchedType === "work" && "rows" in statement) {
      return (statement as WorkStatement).rows as WorkRow[];
    }
    return [];
  })();
  ...
  const payRows: PaymentRow[] = (() => {
    if (!statement) return [];
    if (fetchedType === "combined" && "payment_rows" in statement) return (statement as CombinedStatement).payment_rows;
    if (fetchedType === "payment" && "rows" in statement) {
      return (statement as PaymentStatement).rows as PaymentRow[];
    }
    return [];
  })();
  ...
  // Ledger rows for preview — use fetchedType to match what was actually fetched
  const isTeam = statement?.entity?.entity_type === "team";
  const ledgerRows = (fetchedType === "payment" || fetchedType === "combined")
    ? (() => {
        if (fetchedType === "payment") {
          const allPay = [
        ...
        return buildLedgerRows(workRows, payRows, t);
      })()
    : [];

  // pagination for view
  const allDisplayRows = fetchedType === "work" ? workRows : ledgerRows;
  ...
  return (
    <>
      <title>{`${t("statements.title")} | LabourBook`}</title>
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              ...
              <div>
                <p style={{ fontWeight: 700, fontSize: 17, color: "var(--color-on-surface)" }}>{t("statements.downloadStatementTitle")}</p>
                <p style={{ fontSize: 13, color: "var(--color-on-surface-variant)", marginTop: 2 }}>
                  {statementType === "work" ? t("statements.workStatementLabel") : statementType === "payment" ? t("statements.paymentStatementLabel") : t("statements.combinedLabel")} {t("statements.downloadStatementFor")} <b>{statement.entity.name}</b>
                </p>
              </div>
            </div>
            ...
            <div style={{ backgroundColor: "var(--color-surface-container)", borderRadius: 12, padding: "14px 16px", marginBottom: 24, fontSize: 13, color: "var(--color-on-surface-variant)", lineHeight: 1.7 }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span>{t("statements.period")}</span>
                <span style={{ fontWeight: 600, color: "var(--color-on-surface)" }}>
                  {dateFrom ? fmtDate(dateFrom) : t("statements.allTime")} – {dateTo ? fmtDate(dateTo) : t("statements.today")}
                </span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                <span>{t("statements.type")}</span>
                <span style={{ fontWeight: 600, color: "var(--color-on-surface)" }}>
                  {statementType === "work" ? t("statements.workStatementTitle") : statementType === "payment" ? t("statements.paymentStatementTitle") : t("statements.combinedStatementTitle")}
                </span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                <span>{t("statements.records")}</span>
                <span style={{ fontWeight: 600, color: "var(--color-on-surface)" }}>
                  {allDisplayRows.length} {t("statements.entries")}
                </span>
              </div>
            </div>

            <p style={{ fontSize: 12, color: "var(--color-on-surface-variant)", marginBottom: 20, lineHeight: 1.5 }}>
              {t("statements.printDialogNote")}
            </p>
      ...
      {/* ── PAGE HEADER ── */}
      <header className="px-4 md:px-8 py-6 md:py-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <div className="flex flex-col gap-6">
            ...
            {entitiesLoading ? (
              <div className="flex flex-col items-center py-24 gap-4">
                ...
                <p className="text-body-md" style={{ color: "var(--color-on-surface-variant)" }}>{t("statements.loadingEntities")}</p>
              </div>
            ) : filteredEntities.length === 0 ? (
              <div className="flex flex-col items-center py-20 gap-3">
                <span className="material-symbols-outlined" style={{ fontSize: 56, color: "var(--color-outline)" }}>person_search</span>
                <p className="text-body-md font-semibold" style={{ color: "var(--color-on-surface)" }}>{t("statements.noResultsFound")}</p>
              </div>
            ) : (
              <div>
                <p className="text-label-caps mb-3" style={{ color: "var(--color-on-surface-variant)" }}>{t("statements.selectLabourOrTeam")}</p>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            <span className="text-label-caps px-2 py-0.5 rounded-full" style={{
                              ...
                              {entity.entity_type === "team" ? t("statements.team") : t("statements.labour")}
                            </span>
                            {entity.hometown && <span className="text-label-caps truncate" style={{ color: "var(--color-outline)" }}>{entity.hometown}</span>}
                          </div>
                          <p className="text-label-caps mt-1" style={{ color: "var(--color-on-surface-variant)" }}>₹{entity.daily_wage}{t("statements.perDay")}</p>
                        </div>
        ...
        {step === "configure" && selectedEntity && (
          <div className="max-w-xl mx-auto flex flex-col gap-5">
            ...
            <div className="rounded-xl px-4 py-3 flex items-center gap-3"
              style={{ backgroundColor: "var(--color-surface-container-low)", border: "1px solid var(--color-outline-variant)" }}>
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-body-sm font-bold flex-shrink-0"
                style={{ backgroundColor: "var(--color-primary-container)", color: "var(--color-on-primary)" }}>
                {getInitials(selectedEntity.name)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-body-md font-semibold truncate" style={{ color: "var(--color-on-surface)" }}>{selectedEntity.name}</p>
                <div className="flex gap-2 mt-0.5 flex-wrap items-center">
                  <span className="text-label-caps px-2 py-0.5 rounded-full" style={{
                    ...
                    {selectedEntity.entity_type === "team" ? t("statements.team") : t("statements.individualLabour")}
                  </span>
                  {selectedEntity.hometown && <span className="text-label-caps" style={{ color: "var(--color-outline)" }}>{selectedEntity.hometown}</span>}
                  <span className="text-label-caps" style={{ color: "var(--color-outline)" }}>₹{selectedEntity.daily_wage}{t("statements.perDay")}</span>
                </div>
              </div>
              <button onClick={() => setStep("select")}
                className="text-label-caps px-3 py-1.5 rounded-lg flex-shrink-0"
            ...
            {/* Statement type */}
            <div>
              <p className="text-label-caps mb-2" style={{ color: "var(--color-on-surface-variant)" }}>{t("statements.statementType")}</p>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { value: "work" as StatementType, icon: "assignment", label: t("statements.workStatementLabel"), desc: t("statements.workStatementDesc") },
                  { value: "payment" as StatementType, icon: "account_balance_wallet", label: t("statements.paymentStatementLabel"), desc: t("statements.paymentStatementDesc") },
                  { value: "combined" as StatementType, icon: "summarize", label: t("statements.combinedLabel"), desc: t("statements.combinedDesc") },
                ].map(opt => (
                  <button key={opt.value} id={`stmt-type-${opt.value}`} onClick={() => setStatementType(opt.value)}
                    ...
                    <span className="material-symbols-outlined icon-fill mb-1.5 block" style={{ fontSize: "20px", color: statementType === opt.value ? "var(--color-on-primary)" : "var(--color-on-surface-variant)" }}>
                      {opt.icon}
                    </span>
                    <p className="text-body-sm font-semibold leading-tight" style={{ color: statementType === opt.value ? "var(--color-on-primary)" : "var(--color-on-surface)" }}>{opt.label}</p>
                    <p className="text-label-caps mt-0.5" style={{ fontSize: 9, color: statementType === opt.value ? "var(--color-on-primary-container)" : "var(--color-outline)" }}>{opt.desc}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Date range */}
            <div>
              <p className="text-label-caps mb-3" style={{ color: "var(--color-on-surface-variant)" }}>{t("statements.dateRange")}</p>
              <div className="flex flex-wrap gap-2 mb-4">
                {[
                  { label: t("statements.thisMonth"), fn: () => { setDateFrom(firstOfMonthISO()); setDateTo(todayISO()); } },
                  { label: t("statements.lastMonth"), fn: () => { const d = new Date(); setDateFrom(new Date(d.getFullYear(), d.getMonth()-1, 1).toISOString().slice(0,10)); setDateTo(new Date(d.getFullYear(), d.getMonth(), 0).toISOString().slice(0,10)); } },
                  { label: t("statements.last3Months"), fn: () => { const d = new Date(); setDateFrom(new Date(d.getFullYear(), d.getMonth()-3, 1).toISOString().slice(0,10)); setDateTo(todayISO()); } },
                  { label: t("statements.thisYear"), fn: () => { setDateFrom(`${new Date().getFullYear()}-01-01`); setDateTo(todayISO()); } },
                ].map(p => (
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: t("common.from"), value: dateFrom, onChange: setDateFrom },
                  { label: t("common.to"), value: dateTo, onChange: setDateTo },
                ].map(f => (
                  <div key={f.label}>
                    <label className="text-label-caps block mb-1.5" style={{ color: "var(--color-on-surface-variant)" }}>{f.label}</label>
                    <DatePicker value={f.value} onChange={f.onChange} className="w-full" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <button id="generate-statement-btn" onClick={fetchStatement} disabled={loading}
              ...
              {loading ? (
                <><div className="w-5 h-5 rounded-full border-2 border-t-transparent animate-spin border-white" />{t("statements.generating")}</>
              ) : (
                <><span className="material-symbols-outlined icon-fill" style={{ fontSize: "20px" }}>receipt_long</span>{t("statements.generateStatement")}</>
              )}
            </button>
          </div>
        )}
            ...
            <div style={{
              ...
              <div style={{ marginBottom: 20 }}>
                <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".1em", textTransform: "uppercase", color: "#6b7280", marginBottom: 6 }}>
                  {t("statements.farmBrand")}
                </p>
                <h2 style={{ fontSize: 28, fontWeight: 800, color: "#111827", lineHeight: 1.1 }}>
                  {fetchedType === "work" ? t("statements.workStatementTitle") : fetchedType === "payment" ? t("statements.paymentStatementTitle") : t("statements.combinedStatementTitle")}
                </h2>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 16 }}>
                <div>
                  {[
                    [t("statements.name"), statement.entity.name],
                    [t("statements.entityType"), statement.entity.entity_type === "team" ? t("statements.team") : t("statements.individualLabour")],
                    statement.entity.hometown ? [t("statements.hometown"), statement.entity.hometown] : null,
                    statement.entity.phone ? [t("statements.phone"), statement.entity.phone] : null,
                    [t("statements.dailyWage"), formatCurrency(statement.entity.daily_wage)],
                    statement.entity.car_rent != null ? [t("statements.carRent"), formatCurrency(statement.entity.car_rent)] : null,
                    statement.entity.manager_fee != null ? [t("statements.managerFee"), formatCurrency(statement.entity.manager_fee)] : null,
                    statement.entity.member_count != null ? [t("statements.members"), String(statement.entity.member_count)] : null,
                ...
                <div style={{ textAlign: "right" }}>
                  <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".08em", textTransform: "uppercase", color: "#6b7280" }}>{t("statements.period")}</p>
                  <p style={{ fontSize: 13, fontWeight: 600, color: "#111827", marginTop: 4 }}>
                    {dateFrom ? fmtDate(dateFrom) : t("statements.allTime")} – {dateTo ? fmtDate(dateTo) : t("statements.today")}
                  </p>
                  <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".08em", textTransform: "uppercase", color: "#6b7280", marginTop: 12 }}>{t("statements.generated")}</p>
              ...
              {/* Table — use fetchedType so it matches what was actually loaded */}
              ...
              {totalPages > 1 && (
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <p style={{ fontSize: 12, color: "#9ca3af" }}>
                    {t("statements.showingRecords")
                      .replace("{from}", String((currentPage - 1) * ROWS_PER_PAGE + 1))
                      .replace("{to}", String(Math.min(currentPage * ROWS_PER_PAGE, allDisplayRows.length)))
                      .replace("{total}", String(allDisplayRows.length))}
                  </p>
              ...
              <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 6, padding: "14px 18px", marginBottom: 28 }}>
                <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".08em", textTransform: "uppercase", color: "#6b7280", marginBottom: 8 }}>
                  {t("statements.statementSummary")}
                </p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 24px" }}>
                  {[
                    { label: t("statements.daysPresent"), value: String(statement.summary.total_days_present), color: "#166534" },
                    { label: t("statements.daysAbsent"), value: String(statement.summary.total_days_absent), color: "#991b1b" },
                    { label: t("statements.halfDays"), value: String(statement.summary.total_days_half), color: "#92400e" },
                    { label: t("statements.totalEarned"), value: formatCurrency(statement.summary.total_wage_earned), color: "#166534" },
                    { label: t("statements.totalBorrowed"), value: formatCurrency(statement.summary.total_paid_borrowed), color: "#991b1b" },
                    {
                      label: t("statements.balance"),
                      value: formatCurrency(Math.abs(statement.summary.balance)) + (statement.summary.balance < 0 ? ` (${t("statements.overpaid")})` : statement.summary.balance === 0 ? ` (${t("statements.settled")})` : ` (${t("statements.pending")})`),
              ...
              <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <p style={{ fontSize: 11, color: "#9ca3af" }}>{t("statements.footerComputerGenerated")}</p>
                <p style={{ fontSize: 11, color: "#9ca3af" }}>
                  {t("statements.statementForFooter")} <b style={{ color: "#374151" }}>{statement.entity.name}</b>
                </p>
              </div>
            </div>
          </div>
...
function PreviewWorkTable({ rows, allRows, entity }: { rows: WorkRow[]; allRows: WorkRow[]; entity: EntityInfo }) {
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr>
          <th style={COL_H}>{t("statements.date")}</th>
          <th style={COL_H}>{t("statements.status")}</th>
          <th style={COL_H}>{t("statements.taskDescription")}</th>
          <th style={{ ...COL_H, textAlign: "center" }}>{isTeam ? t("statements.workers") : t("statements.hours")}</th>
          <th style={{ ...COL_H, textAlign: "right" as const }}>{t("statements.wageEarned")}</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((r, i) => {
      ...
      <tfoot>
        <tr style={{ background: "#f0fdf4", borderTop: "2px solid #1b4332" }}>
          <td colSpan={4} style={{ padding: "11px 14px", fontSize: 13, fontWeight: 700, color: "#1b4332" }}>{t("statements.totalWageEarned").toUpperCase()} ({allRows.length} {t("statements.records").toLowerCase()})</td>
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr>
          <th style={COL_H}>{t("statements.date")}</th>
          {showTypeCol && <th style={COL_H}>{t("statements.typeColumn")}</th>}
          <th style={COL_H}>{t("statements.description")}</th>
          <th style={{ ...COL_H, textAlign: "right" as const }}>{t("statements.creditedPlus")}</th>
          <th style={{ ...COL_H, textAlign: "right" as const }}>{t("statements.borrowedMinus")}</th>
          <th style={{ ...COL_H, textAlign: "right" as const }}>{t("statements.balance")}</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((r, i) => {
          ...
          return (
            <tr key={`${r.date}-${i}`} style={{ borderBottom: "1px solid #e5e7eb", background: i % 2 === 0 ? "#fff" : "#f8faf9" }}>
              ...
              {showTypeCol && (
                <td style={{ padding: "9px 14px" }}>
                  {r.rowType === "work"
                    ? <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 7px", borderRadius: 9999, background: "#dcfce7", color: "#166534" }}>{t("statements.wage")}</span>
                    : <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 7px", borderRadius: 9999, background: "#fee2e2", color: "#991b1b" }}>{t("statements.advance")}</span>
                  }
                </td>
              )}
      ...
      <tfoot>
        <tr style={{ background: "#f0fdf4", borderTop: "2px solid #1b4332" }}>
          <td colSpan={showTypeCol ? 3 : 2} style={{ padding: "11px 14px", fontSize: 13, fontWeight: 700, color: "#1b4332" }}>
            {t("statements.totals").toUpperCase()} ({allRows.length} {t("statements.records").toLowerCase()})
          </td>
